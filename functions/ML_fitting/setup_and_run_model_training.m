...
if DoTestPerformances==1
    ...
    if diagnostics==1
        ...
        %title('Fitting Error (normalized by the upper bound values)')
        ...
        ylim([0 1])
        %title('Adjusted R squared of linear fit')
        ...
        ylim([0 4.0])
        %title('Root mean squared error (normalized by the upper bound values)')

        %legend({'NLLS (GT)', 'NLLS (Grid Search)', 'ML'})
        T = getframe(h);
        ...
        for param_to_test = 1:5
            ...
            %legend({'Ideal case', 'NLLS (GT)', 'NLLS (Grid Search)', 'ML'});
