function SANDIinput = setup_and_run_model_training_with_dot_forGUI(SANDIinput, fig_ax)
...
model.Dsoma = Dsoma;
model.paramsrange = [0 1; 0 1; 0.05 Din_UB; 1 Rsoma_UB; 0.05 De_UB; 0 0.20];
...
disp(['   - fdot = [' num2str(model.paramsrange(6,1)) ', ' num2str(model.paramsrange(6,2)) ']'])
fprintf(SANDIinput.LogFileID,['   - fdot = [' num2str(model.paramsrange(6,1)) ', ' num2str(model.paramsrange(6,2)) ']\n']);
...
[~, database_train_noisy, params_train, sigma_mppca, bvals, Ndirs_per_Shell] = build_training_set_with_dot(SANDIinput);
model.bvals = bvals;
...
ffit = @(p,x) cos(p(1)).^2.*fstick(p(3),x) + ...
    ...
    (1 - cos(p(1)).^2 - (1-cos(p(1)).^2).*cos(p(2)).^2).*fball(p(5),x) + p(6);

model.fmodel = ffit;

SANDIinput.model = model;

f_rm = @(p,x) RiceMean(ffit([p(1),p(2),p(3),p(4),p(5),p(6)],x),p(7));
...
if model.boost
    ...
    parfor i = 1:size(database_train_noisy,1)
        ...
        params_train_NLLS(i,:) = lsqcurvefit(@(p,x) f_rm([p(1), p(2), p(3), p(4), p(5), p(6), sigma_mppca(i)],x), xini(i,:), bvals, database_train_noisy(i,:), lb, ub, options);
...
if DoTestPerformances==1
    ...
    [~, database_test_noisy, params_test, sigma_mppca_test, bvals] = build_training_set_with_dot(SANDIinput);
    ...
    parfor i = 1:size(database_test_noisy,1)
        ...
        bestest_NLLS(i,:) = lsqcurvefit(@(p,x) f_rm([p(1), p(2), p(3), p(4), p(5), p(6), sigma_mppca_test(i)],x), xini0(i,:), bvals, database_test_noisy(i,:), lb, ub, options);
        bestest_NLLS_noisy(i,:) = lsqcurvefit(@(p,x) f_rm([p(1), p(2), p(3), p(4), p(5), p(6), sigma_mppca_test(i)],x), xini(i,:), bvals, database_test_noisy(i,:), lb, ub, options);
    ...
    fprintf(SANDIinput.LogFileID,['DONE - NLLS estimation took ' num2str(round(tt)) ' sec.\n']);
    % Calculate the MSEs
    mse_nlls = mean((bestest_NLLS(:,1:6) - params_test).^2,1);
    mse_nlls_noisy = mean((bestest_NLLS_noisy(:,1:6) - params_test).^2,1);
    mse_ml = mean((MLpredictions(:,1:6) - params_test).^2,1);

    try
        current_ax = subplot(1,1,1,'Parent', fig_ax{2});
        corrplot(current_ax, bestest_NLLS(:,1:6),'varNames',{'fneurite','fsoma','Din','Rsoma','De','fdot'}, 'type','Spearman', 'testR','on')
    ...
    error_nlls = (bestest_NLLS(:,1:6) - params_test)./repmat(max_values',[model.Nset 1]);
    error_nlls_noisy = (bestest_NLLS_noisy(:,1:6) - params_test)./repmat(max_values',[model.Nset 1]);
    error_ml = (MLpredictions(:,1:6) - params_test)./repmat(max_values',[model.Nset 1]);
