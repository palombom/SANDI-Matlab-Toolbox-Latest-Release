...
names = {'fneurite', 'fsoma', 'Din', 'Rsoma', 'De', 'fextra', 'Rsoma_Filtered', 'Din_Filtered', 'De_Filtered', 'fsoma_Filtered', 'fneurite_Filtered'};
...
try
    ...
    for i=1:size(mpgMean,2)+3
        ...
        elseif i==size(mpgMean,2)+2
            Rsoma_tmp = mpgMean(:,4);
            Rsoma_tmp(fextra>0.45) = 0;
            itmp(mask==1) = Rsoma_tmp;
        elseif i==size(mpgMean,2)+3
            Din_tmp = mpgMean(:,3);
            Din_tmp(fextra>0.45) = 0;
            itmp(mask==1) = Din_tmp;
        elseif i==size(mpgMean,2)+4
            De_tmp = mpgMean(:,5);
            De_tmp(fextra>0.45) = 0;
            itmp(mask==1) = De_tmp;
        elseif i==size(mpgMean,2)+5
            fsoma_tmp = fsoma;
            fsoma_tmp(fextra>0.45) = 0;
            itmp(mask==1) = fsoma_tmp;
        elseif i==size(mpgMean,2)+6
            fneurite_tmp = fneurite;
            fneurite_tmp(fextra>0.45) = 0;
            itmp(mask==1) = fneurite_tmp;
        else
        ...
        if diagnostics==1
            if i<=size(mpgMean,2)+1
                h = figure('Name',['SANDI ' names{i} ' map'],'Position',get(0,'Screensize')); hold on
                %tmp = imtile(itmp);
                tmp = itmp(:,:,round(size(itmp,3)/2));
