...
fprintf(SANDIinput.LogFileID,'* Computing the Spherical Mean for dataset: %s\n', data_filename); 

if isempty(noisemap_mppca_filename) && ~isempty(SANDIinput.SNR) && SANDIinput.SNR > 0
    disp(['    WARNING: Noisemap was not provided; using the provided average SNR = ' num2str(SNR)])
    fprintf(SANDIinput.LogFileID, '    WARNING: Noisemap was not provided; using the provided average SNR = %s\n', num2str(SNR));
elseif isempty(SANDIinput.noisemap_mppca_filename) && (isempty(SANDIinput.SNR) || SANDIinput.SNR == 0)
    SANDIinput.SNR = 35;
    disp(['    WARNING: Noisemap was not provided; average SNR was not provided; using nominal average SNR = ' num2str(SNR)])
    fprintf(SANDIinput.LogFileID, '    WARNING: Noisemap was not provided; average SNR was not provided; using nominal average SNR = %s\n', num2str(SNR));
end
...
fprintf(SANDIinput.LogFileID, '   - Loading the data\n');
...
for i=1:numel(bunique)
    ...
    elseif i>1
        ...
        if                Ndir<=9,   order = 1;
        elseif Ndir>9  && Ndir<=15,  order = 2;
        elseif Ndir>15 && Ndir<=28,  order = 4;
        elseif Ndir>28 && Ndir<=45,  order = 6;
        elseif Ndir>45,              order = 8;
        end
        
        disp(['   - Fitting SH to the per-shell data with: b=' num2str(bunique(i)) ' s/mm^2; order l=' num2str(order) ' - shell ' num2str(i-1) ' of ' num2str(numel(bunique)-1) ' ; #directions = ' num2str(sum(bvals==bunique(i)))])
        fprintf(SANDIinput.LogFileID, ['   - Fitting SH to the per-shell data with: b=' num2str(bunique(i)) ' s/mm^2; order l=' num2str(order) ' - shell ' num2str(i-1) ' of ' num2str(numel(bunique)-1) ' ; #directions = ' num2str(sum(bvals==bunique(i))) '\n']);
...
if ~isempty(noisemap_mppca_filename)
    ...
    noisemap_mppca = 1./SNR.*ones(size(estimated_sigma));
end
...
if diagnostics==1
    ...
    T = getframe(h);
    mkdir(fullfile(output_folder , 'SANDIreport'));
...
fprintf(SANDIinput.LogFileID,['DONE - Spherical mean computed in ' num2str(round(tt)) ' sec.\n']);
end