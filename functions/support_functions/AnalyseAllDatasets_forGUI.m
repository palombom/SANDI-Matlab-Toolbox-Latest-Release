function SANDIinput = AnalyseAllDatasets_forGUI(SANDIinput, fig_axes)
...
for subj_id = 1:numel(SubList)
    ...
    for ses_id = 1:numel(SesList)
        ...
        if SANDIinput.WithDot==1
                run_model_fitting_with_dot_forGUI(SANDIinput,fig_axes);
        else
                run_model_fitting_forGUI(SANDIinput,fig_axes);
        end
    end
end
end