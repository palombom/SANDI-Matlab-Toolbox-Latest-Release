function SANDIinput = make_direction_average_forGUI(SANDIinput, fig_ax)
...
if isempty(noisemap_mppca_filename)
    disp(['    Noisemap was not provided; using the provided average SNR = ' num2str(SNR)])
    fprintf(SANDIinput.LogFileID, '    Noisemap was not provided; using the provided average SNR=%s\n', num2str(SNR));
end
...
for i=1:numel(bunique)
    ...
    elseif i>1
        ...
        fprintf(SANDIinput.LogFileID, ['   - Fitting SH to the per-shell data with order l=' num2str(order) ' - shell ' num2str(i-1) ' of ' num2str(numel(bunique)-1) ' ; #directions = ' num2str(sum(bvals==bunique(i))) '\n']);
...
if diagnostics==1
    ...
    for vv = 1:size(img_to_show,3)
        current_ax = subplot(4,4,vv, 'Parent', fig_ax);
        imshow(img_to_show(:,:,vv),[0 0.7], 'Parent', current_ax),
        title(current_ax, ['b = ' num2str(bunique(vv)) ' s/mm^2'])
    end
end
